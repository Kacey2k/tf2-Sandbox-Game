...
if (target != noone && point_distance(x, y, target.x, target.y) <= shooting_range) {
    ...
    if (obstruction == noone) {
        ...
        scr_movement_combat_pyro();

        if (can_shoot) {    // Shoot if cooldown is done
            var bullet = instance_create_layer(x, y, "Instances", blu_fire);
        ...
        // Obstruction detected, use normal movement
        scr_movement_pyro();
    }
} else {
    scr_movement_pyro();
...
if (wholeMove > 0) && (isUbered = true) {
	sprite_index = spr_blu_pyro_moving_ubered;
	isChoking = false;
} else if (wholeMove = 0) && (isUbered = true) {
	sprite_index = spr_blu_pyro_ubered;
	audio_stop_sound(walk_sound_instance);
} else if (wholeMove > 0) && (isUbered = false) {
	sprite_index = spr_blu_pyro_moving;
	isChoking = false;
} else {
	sprite_index = spr_blu_pyro;
	audio_stop_sound(walk_sound_instance);
}
...
if (isUbered = true) {
	hp = maxhp;
	isBurning = false; //potentially redundant
}
